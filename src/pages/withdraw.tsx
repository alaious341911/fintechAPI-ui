...
import type {Deposit, LinkBankAccount, Withdraw} from '../interfaces'
...
const WithdrawPage = () => {
    const DEPOSIT = "/api/withdraw";
    
 let token=''

    const newLinking: Withdraw ={
      amount: 0
    }
...
console.log("Withdraw Money", values)
      ...
      try {
          ...
          const response = await axios.post(DEPOSIT,
            ...
            if(response?.status == 200){
              ...
              toast.update(id, { render: "withdraw successful!", type: "success", 
  ...
  return (
    <>
      <Head>
        <title>{getPageTitle('Withdraw from Account')}</title>
      </Head>

      <SectionTitle>
        <p style={dashboardHeading}>withdraw from Account</p>
      </SectionTitle>
